@props(['client', 'blogKeywords', 'importRoute', 'exportRoute', 'deleteRoute', 'indexRoute', 'indexLabel'])
...
<div class="space-y-6">
    ...
    <x-admin.card :title="__('Import / Export')">
        ...
        <p class="mt-3 text-xs text-gray-500">
            {{ __('One keyword per row. Duplicate keywords already on this list are skipped automatically.') }}
        </p>
    </x-admin.card>

    <x-admin.card :title="__('Blog Keywords')">
        @if ($blogKeywords->isEmpty())
            <p class="text-sm text-gray-500">{{ __('No blog keywords yet. Import a CSV above to add some.') }}</p>
        @else
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($blogKeywords as $blogKeyword)
                            <tr>
                                <td class="px-4 sm:px-6 py-2.5 font-medium text-gray-900">{{ $blogKeyword->keyword }}</td>
                                <td class="px-4 sm:px-6 py-2.5 text-right">
                                    <button type="submit" form="delete-blog-keyword-{{ $blogKeyword->id }}" class="text-red-600 hover:text-red-800 font-medium">{{ __('Delete') }}</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            {{-- One hidden form per blog keyword, referenced by the Delete button above via the `form="..."` attribute. --}}
            @foreach ($blogKeywords as $blogKeyword)
                <form
                    id="delete-blog-keyword-{{ $blogKeyword->id }}" method="POST" action="{{ route($deleteRoute, [$client, $blogKeyword]) }}" class="hidden"
                    x-data
