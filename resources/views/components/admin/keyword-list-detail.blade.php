...
<div class="space-y-6">
    ...
    <x-admin.card :title="__('Import / Export')">
        ...
        <p class="mt-3 text-xs text-gray-500">
            {{ __('One keyword per row. Use Google Ads match-type syntax if needed: [exact], "phrase", or bare text for broad. Duplicate keywords already on this list are skipped automatically.') }}
        </p>
    </x-admin.card>

    <x-admin.card :title="__('Keywords')">
        @if ($keywords->isEmpty())
            <p class="text-sm text-gray-500">{{ __('No keywords yet. Import a CSV above to add some.') }}</p>
        @else
